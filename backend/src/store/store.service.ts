...
export class StoreService implements OnModuleInit {
  ...
  addAudit(action: string, actor: string, details: string) {
  ...
  private seed() {
    // Template type each newly-created time slot represents, keyed by slot id.
    // This is seed-time-only knowledge — TimeSlot itself carries no type; the
    // Time Slot Engine defines structure only. "Type" (lesson/break/lunch/
    // assembly/activity) is a per-class attribute set on MasterTimetableEntry,
    // configured per class from the Master Timetable screen.
    const slotTemplateType = new Map<string, SlotType>();
    this.schoolConfig = {
    ...
    // ---- Academic Terms ----
    const term1: Term = { id: this.id(), academicYear: '2026', name: 'Term 1', startDate: '2026-01-12', endDate: '2026-04-03', active: false };
    const term2: Term = { id: this.id(), academicYear: '2026', name: 'Term 2', startDate: '2026-05-04', endDate: '2026-08-14', active: false };
    const term3: Term = { id: this.id(), academicYear: '2026', name: 'Term 3', startDate: '2026-09-01', endDate: '2026-12-11', active: true };
    this.terms.push(term1, term2, term3);
    this.currentTermId = term3.id;

    // ---- Time slots (structure only — start/end/label; no type) ----
    const mkSlot = (
      day: Day,
      order: number,
      start: string,
      ...
      const slot: TimeSlot = { id: this.id(), day, order, start, end, label };
    ...
    for (const day of WORKING_DAYS) {
      ...
      if (day === 'MON') {
        this.timeSlots.push(
          mkSlot(day, order++, '07:30', '07:45', 'ASSEMBLY', 'Assembly'),
        );
      }
      this.timeSlots.push(
        mkSlot(day, order++, '07:45', '08:25', 'LESSON', 'Period 1'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '08:25', '09:05', 'LESSON', 'Period 2'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '09:05', '09:20', 'BREAK', 'Interval'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '09:20', '10:00', 'LESSON', 'Period 3'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '10:00', '10:40', 'LESSON', 'Period 4'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '10:40', '11:10', 'LUNCH', 'Lunch'),
      );
      this.timeSlots.push(
        mkSlot(day, order++, '11:10', '11:50', 'LESSON', 'Period 5'),
      );
      if (day !== 'FRI') {
        this.timeSlots.push(
          mkSlot(day, order++, '11:50', '12:30', 'LESSON', 'Period 6'),
        );
      }
    }

    // ---- Grades & classes ----
    const g9: Grade = { id: this.id(), name: 'Grade 9', order: 1, active: true };
    const g10: Grade = {
      id: this.id(),
      name: 'Grade 10',
      order: 2,
    ...
    const mkClass = (name: string, gradeId: string, count: number): SchoolClass => ({
      id: this.id(),
      name,
      gradeId,
      studentCount: count,
    ...
    // ---- Rooms ----
    const room = (name: string, type: Room['type'], capacity: number): Room => ({
      id: this.id(),
      name,
      type,
      capacity,
    ...
    for (const g of [g9, g10, g11]) {
      this.gradeSubjects.push(
        gsc(g.id, MATH.id, 5, 2),
        gsc(g.id, SCI.id, 4, 2),
        gsc(g.id, ENG.id, 5, 2),
        gsc(g.id, HIST.id, 3, 1),
        gsc(g.id, ICT.id, 2, 1),
        gsc(g.id, SIN.id, 4, 2),
        gsc(g.id, PE.id, 3, 2),
        gsc(g.id, ART.id, 3, 1),
    ...
    // Demo teacher availability restriction — Nadeeka has a standing
    // commitment during Monday Period 1, so relief search must exclude her then.
    const monP1 = this.timeSlots.find((s) => s.day === 'MON' && s.label === 'Period 1');
    if (monP1) T13.unavailable = [{ day: 'MON', periodId: monP1.id }];

    g10.headTeacherId = T5.id;

    // ---- Users (RBAC demo accounts) ----
    this.users.push(
      { id: this.id(), name: 'Admin User', email: 'admin@lnc.edu', role: 'ADMIN' },
      { id: this.id(), name: 'Mrs. Fernando (Principal)', email: 'principal@lnc.edu', role: 'PRINCIPAL' },
      { id: this.id(), name: 'Mr. Jayawardena (VP)', email: 'vp@lnc.edu', role: 'VICE_PRINCIPAL' },
      { id: this.id(), name: 'Ms. Wickramasinghe (Sectional Head)', email: 'sectional@lnc.edu', role: 'SECTIONAL_HEAD', gradeIds: [g9.id, g10.id, g11.id] },
      { id: this.id(), name: 'Sanduni Jayasuriya (Grade Head - 10)', email: 'sanduni.j@lnc.edu', role: 'GRADE_HEAD', gradeIds: [g10.id], teacherId: T5.id },
      { id: this.id(), name: 'Priya Perera (Class Teacher 9-A)', email: 'priya.perera@lnc.edu', role: 'CLASS_TEACHER', teacherId: T1.id },
      { id: this.id(), name: 'Kamal Fernando (Teacher)', email: 'kamal.fernando@lnc.edu', role: 'TEACHER', teacherId: T3.id },
      { id: this.id(), name: 'Dilani Wickrama (Teacher)', email: 'dilani.w@lnc.edu', role: 'TEACHER', teacherId: T7.id },
      { id: this.id(), name: 'Mr. Silva (Parent of 9-A)', email: 'silva.parent@example.com', role: 'PARENT', classId: c9a.id },
    );
    ...
    // ---- Seed today's daily timetable + a couple of demo absences ----
    ...
    // Demo absence #1: Dilani Wickrama (History, all classes) - whole day, pending
    const histEntries = this.dailyTimetable.filter(
      (e) => e.date === dateStr && e.originalTeacherId === T7.id,
    );
    if (histEntries.length) {
      const absence1: Absence = {
        id: this.id(),
        teacherId: T7.id,
        date: dateStr,
        wholeDay: true,
      ...
      this.addAudit(
        'ABSENCE_SUBMITTED',
        T7.name,
        `${T7.name} marked absent for the whole day on ${dateStr} (Sick Leave). ${histEntries.length} lessons affected.`,
      );
      this.addNotification(
        `${T7.name} is absent today (Sick Leave). ${histEntries.length} lessons need relief.`,
        'ABSENCE',
        'VICE_PRINCIPAL',
      );
    }

    // Demo absence #2: Kamal Fernando (Science) - 2 periods, already approved + relief assigned
    ...
    if (kamalEntries.length) {
      ...
      // auto-assign a relief teacher (T14 floating science/english)
      ...
      this.addAudit(
        'RELIEF_ASSIGNED',
        'Admin User',
        `Assigned ${T14.name} as relief for ${T3.name}'s class on ${dateStr}.`,
      );
    }

    // ---- Seed lesson plans (a spread of statuses so the monitor view has something to show) ----
    ...
    if (priyaToday) {
      // Demonstrate a double period: include a second matching slot for the same class/subject/day if one exists.
      const priyaToday2 = this.masterTimetable.find(
        (e) => e.day === todayDay && e.teacherId === T1.id && e.type === 'LESSON' && e.classId === priyaToday.classId
    ...
    const overdueSrc = this.masterTimetable.find(
      (e) => e.teacherId === T3.id && e.type === 'LESSON' && otherDays.includes(e.day) && e.id !== kamalToday?.id,
    );
    if (overdueSrc) {
      this.lessonPlans.push({
  ...
  /**
   * Builds the master timetable from the Time Slot Engine's structural template.
   * Non-lesson slots (assembly/break/lunch) are auto-applied to every class as
   * the shared default template; lesson slots are filled by generateLessons().
   * Either kind remains editable per class afterwards from the Master Timetable
   * screen, or re-run via the Generate Timetable feature.
   */
  private generateMasterTimetable(slotTemplateType: Map<string, SlotType>) {
    for (const [slotId, templateType] of slotTemplateType) {
      if (templateType === 'LESSON') continue;
  ...
  /**
   * On-demand assisted timetable generation ("Generate Timetable" feature).
   * Fills LESSON periods for the given classes according to their grade's
   * subject/period-per-week configuration, teacher-subject-grade/class
   * allocation, and teacher availability — guaranteeing no teacher, class or
   * room conflict with the rest of the school's existing schedule.
   *
   * mode 'FILL_GAPS' only touches slots that are currently completely empty
   * for that class (never disturbs locked or already-scheduled periods).
   * mode 'REGENERATE' first clears that class's unlocked LESSON periods, then
   * fills gaps as above — giving a clean rebuild while still respecting locks.
   */
  generateLessons(targetClassIds: string[], mode: 'FILL_GAPS' | 'REGENERATE') {
    ...
    if (mode === 'REGENERATE') {
      ...
      this.masterTimetable = this.masterTimetable.filter(
        (e) => !(targetSet.has(e.classId) && e.type === 'LESSON' && !e.locked),
      );
    ...
    // The greedy fill below has no backtracking, so a tightly-packed subject
    // configuration can occasionally leave a period or two unresolved on a
    // single pass even though a fully-resolved arrangement exists. Retry with
    // a fresh random ordering each time and keep the best (ideally zero-gap)
    // result, instead of leaving it to luck which attempt the user gets.
    const baseline = [...this.masterTimetable];
    const MAX_ATTEMPTS = 30;
    let best: { timetable: MasterTimetableEntry[]; created: number; unresolved: string[] } | null = null;
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (result.unresolved.length === 0) break;
    }
  ...
  /** Single greedy pass: fills empty lesson slots for targetClasses into this.masterTimetable. */
  private fillLessonsOnce(targetClasses: SchoolClass[]) {
    const subjectById = new Map(this.subjects.map((s) => [s.id, s]));
    const gradeSubjectConf = (gradeId: string, subjectId: string) =>
      this.gradeSubjects.find((g) => g.gradeId === gradeId && g.subjectId === subjectId);
    const qualifiedTeachers = (subjectId: string, classId: string, gradeId: string) =>
    ...
    // Periods still needed per class/subject, net of whatever is already scheduled (kept or locked).
    ...
    // Busy sets from the ENTIRE existing schedule (school-wide) so a scoped/partial
    // regeneration never conflicts with classes or days outside its scope.
    const teacherBusy = new Set<string>(); // `${teacherId}|${periodId}`
    const roomBusy = new Set<string>(); // `${roomId}|${periodId}`
    const occupied = new Set<string>(); // `${classId}|${periodId}` — anything already there at all
    const perDayCount = new Map<string, Map<string, number>>(); // `${classId}|${day}` -> subjectId -> count
    for (const e of this.masterTimetable) {
      if (e.teacherId) teacherBusy.add(`${e.teacherId}|${e.periodId}`);
      if (e.roomId) roomBusy.add(`${e.roomId}|${e.periodId}`);
      occupied.add(`${e.classId}|${e.periodId}`);
      if (e.subjectId) {
        const key = `${e.classId}|${e.day}`;
        if (!perDayCount.has(key)) perDayCount.set(key, new Map());
    ...
    // A fresh random seed per call means two consecutive Full Regenerate runs on
    // identical input produce a genuinely different (but still conflict-free)
    // arrangement, instead of silently reproducing the exact same schedule.
    ...
    for (const day of shuffledDays) {
      ...
      for (const slot of slots) {
        ...
        for (const cls of ordered) {
          ...
          if (occupied.has(occKey)) continue; // already has a template/locked/kept entry

          const remMap = remaining.get(cls.id)!;
          const dayKey = `${cls.id}|${day}`;
          if (!perDayCount.has(dayKey)) perDayCount.set(dayKey, new Map());
          ...
          for (const [subjectId] of candidates) {
            ...
            if (conf && usedToday >= conf.maxPeriodsPerDay) continue;

            const subject = subjectById.get(subjectId)!;
            const prefScore = (teacherId: string) => {
            ...
            let roomId: string | undefined = cls.roomId;
            if (subject.requiresSpecialRoom) {
              const specialRoom = this.rooms.find(
                (r) =>
                  (subject.code === 'ICT' && r.name === 'ICT Lab') ||
            ...
            if (roomId) roomBusy.add(`${roomId}|${slot.id}`);
  ...
  materializeDailyTimetable(date: string, day: Day) {
    const existing = this.dailyTimetable.some((e) => e.date === date);
    if (existing) return;
    if (this.isHoliday(date)) return; // no periods materialize on a full holiday
