...
import { LESSON_LIKE_TYPES, MasterTimetableEntry } from '../../common/types.js';
...
export class TimetableController {
  ...
  @Get('master')
  listMaster(@Query('classId') classId?: string, @Query('day') day?: string) {
  ...
  private conflictCheck(entry: Partial<MasterTimetableEntry>, ignoreId?: string) {
    const clashes = this.store.masterTimetable.filter(
      (e) =>
        e.id !== ignoreId &&
        e.day === entry.day &&
  ...
  private availabilityConflict(entry: Partial<MasterTimetableEntry>) {
  ...
  @Post('master')
  create(@Body() body: Partial<MasterTimetableEntry> & { directPublish?: boolean; proposedBy?: string }) {
  ...
  @Put('master/:id')
  update(@Param('id') id: string, @Body() body: Partial<MasterTimetableEntry> & { directPublish?: boolean; proposedBy?: string }) {
  ...
  private queuePendingChange(entryId: string | undefined, proposedChange: Partial<MasterTimetableEntry>, proposedBy: string) {
  ...
  decidePending(@Param('id') id: string, @Body() body: { approve: boolean; decidedBy: string }) {
    const change = this.store.pendingTimetableChanges.find((c) => c.id === id);
    if (!change) throw new NotFoundException();
    if (change.status !== 'PENDING') throw new BadRequestException('This change has already been decided.');
    ...
    if (body.approve) {
      if (change.entryId) {
        ...
        this.store.masterTimetable.push({ ...change.proposedChange, id: this.store.id() } as MasterTimetableEntry);
      }
    }

    this.store.addAudit(
      'TIMETABLE_CHANGE_DECIDED',
      body.decidedBy,
      `${body.decidedBy} ${body.approve ? 'approved' : 'rejected'} a master timetable change proposed by ${change.proposedBy}.`,
    );
    return change;
  }
  ...
  @Post('group-lesson')
  createGroupLesson(
    @Body() body: { day: MasterTimetableEntry['day']; periodId: string; groupId: string; teacherId: string; roomId?: string; locked?: boolean },
  ) {
    ...
    if (!group.memberClassIds.length) throw new BadRequestException('This group has no member classes.');

    // Check every member class for a clash before creating anything (all-or-nothing).
    ...
    this.store.addAudit(
      'GROUP_LESSON_CREATED',
      'Admin User',
      `Group lesson "${group.name}" scheduled on ${body.day} across ${group.memberClassIds.length} classes.`,
    );
    return entries;
  }

  @Delete('group-lesson/:groupId/:day/:periodId')
  removeGroupLesson(@Param('groupId') groupId: string, @Param('day') day: string, @Param('periodId') periodId: string) {
    const before = this.store.masterTimetable.length;
    this.store.masterTimetable = this.store.masterTimetable.filter(
      (e) => !(e.groupId === groupId && e.day === day && e.periodId === periodId),
    );
  ...
  remove(@Param('id') id: string) {
    const entry = this.store.masterTimetable.find((e) => e.id === id);
    if (entry?.locked) throw new BadRequestException('Entry is locked.');
  ...
  validate() {
    ...
    // teacher/class/room conflicts (should be none by construction, but re-check)
    ...
    for (const [key, entries] of seen) {
      // Entries that all belong to the same group lesson (same groupId) are expected
      // to share a teacher/room at the same day+period — that's one merged session
      // taught across several classes, not a conflict.
    ...
    // required periods/week satisfied
    for (const cls of this.store.classes) {
      ...
      for (const conf of configs) {
        const actual = this.store.masterTimetable.filter(
          (e) => e.classId === cls.id && e.subjectId === conf.subjectId,
        ).length;
    ...
    for (const teacher of this.store.teachers) {
      ...
      const violations = this.store.masterTimetable.filter(
        (e) => e.teacherId === teacher.id && teacher.unavailable.some((u) => u.day === e.day && u.periodId === e.periodId),
      );
    ...
    // max consecutive periods per teacher per day
    for (const teacher of this.store.teachers) {
  ...
  @Post('generate')
  generate(@Body() body: { scope: 'ALL' | 'CLASS'; classId?: string; mode: 'FILL_GAPS' | 'REGENERATE' }) {
    if (body.scope === 'CLASS' && !body.classId) {
      throw new BadRequestException('classId is required when scope is CLASS');
    }
    const targetClassIds =
      body.scope === 'ALL' ? this.store.classes.map((c) => c.id) : [body.classId!];

    const invalid = targetClassIds.find((id) => !this.store.classes.some((c) => c.id === id));
    if (invalid) throw new NotFoundException('Class not found');

    const result = this.store.generateLessons(targetClassIds, body.mode);

    const scopeLabel =
      body.scope === 'ALL' ? 'the entire school' : this.store.classes.find((c) => c.id === body.classId)?.name ?? 'a class';
    this.store.addAudit(
      'TIMETABLE_GENERATED',
      'Admin User',
  ...
  @Post('copy-day')
  copyDay(@Body() body: { classId: string; fromDay: string; toDay: string }) {
    const { classId, fromDay, toDay } = body;
    if (!classId || !fromDay || !toDay) throw new BadRequestException('classId, fromDay and toDay are required');
    if (fromDay === toDay) throw new BadRequestException('Source and target day must differ');

    const sourceEntries = this.store.masterTimetable.filter((e) => e.classId === classId && e.day === fromDay);
    const targetSlotsByLabel = new Map(
      this.store.timeSlots.filter((s) => s.day === toDay).map((s) => [s.label, s]),
    );
    ...
    for (const src of sourceEntries) {
      ...
      const existing = this.store.masterTimetable.find(
        (e) => e.classId === classId && e.day === toDay && e.periodId === targetSlot.id,
      );
      if (existing?.locked) {
      ...
      const candidate: Partial<MasterTimetableEntry> = {
        day: toDay as MasterTimetableEntry['day'],
      ...
      if (existing) {
        ...
        this.store.masterTimetable.push({ ...candidate, id: this.store.id(), locked: false } as MasterTimetableEntry);
      }
      copied += 1;
    }
    ...
    this.store.addAudit(
      'MASTER_DAY_COPIED',
      'Admin User',
      `Copied ${cls?.name}'s ${fromDay} schedule to ${toDay} — ${copied} period(s) copied, ${skipped.length} skipped.`,
    );
    return { copied, skipped };
  }

  @Post('publish')
  publish(@Body() body: { notes?: string }) {
