// Browser-side port of backend/src/store/store.service.ts — same seed data and
// business logic (timetable generation, daily materialization, etc.), adapted
// to run without Nest.js DI/decorators so it can back an in-browser mock API
// for the GitHub Pages static deploy (see mock/adapter.ts).
import type {
  Absence,
  Assignment,
  AuditLogEntry,
...
const WORKING_DAYS: Day[] = ['MON', 'TUE', 'WED', 'THU', 'FRI'];
...
class MockStore {
  ...
  addNotification(message: string, category: string, toRole?: Role, toUserId?: string) {
  ...
  private seed() {
    ...
    const mkSlot = (day: Day, order: number, start: string, end: string, templateType: SlotType, label: string): TimeSlot => {
    ...
    const g10: Grade = { id: this.id(), name: 'Grade 10', order: 2, active: true };
    const g11: Grade = { id: this.id(), name: 'Grade 11', order: 3, active: true };
    this.grades.push(g9, g10, g11);

    const mkClass = (name: string, gradeId: string, count: number): SchoolClass => ({ id: this.id(), name, gradeId, studentCount: count });
    ...
    const room = (name: string, type: Room['type'], capacity: number): Room => ({ id: this.id(), name, type, capacity });
    ...
    const subj = (name: string, code: string, requiresSpecialRoom = false, allowConsecutive = false): Subject => ({
      id: this.id(), name, code, requiresSpecialRoom, allowConsecutive,
    });
    ...
    const gsc = (gradeId: string, subjectId: string, perWeek: number, maxPerDay: number): GradeSubjectConfig => ({
      id: this.id(), gradeId, subjectId, periodsPerWeek: perWeek, maxPeriodsPerDay: maxPerDay,
    });
    ...
    const mkTeacher = (name: string, email: string, employeeNo: string, subjectIds: string[], gradeIds: string[], classIds: string[]): Teacher => ({
      id: this.id(), name, email, phone: '077' + Math.floor(1000000 + Math.random() * 8999999),
      employeeNo, subjectIds, gradeIds, classIds,
      maxPeriodsPerDay: 6, maxPeriodsPerWeek: 26, maxConsecutivePeriods: 3, unavailable: [], active: true,
    });
    ...
    this.timetableVersions.push({
      id: this.id(), version: 1, status: 'PUBLISHED', createdAt: new Date().toISOString(), createdBy: 'Admin User',
      publishedAt: new Date().toISOString(), notes: 'Initial published master timetable for demo academic year 2026.',
    });
    ...
    const dayMap: Record<number, Day | null> = { 0: null, 1: 'MON', 2: 'TUE', 3: 'WED', 4: 'THU', 5: 'FRI', 6: null };
    ...
    if (histEntries.length) {
      const absence1: Absence = {
        id: this.id(), teacherId: T7.id, date: dateStr, wholeDay: true,
        periodIds: histEntries.map((e) => e.periodId), reason: 'Sick Leave', remarks: 'Fever, advised rest by doctor.',
    ...
    const kamalEntries = this.dailyTimetable.filter((e) => e.date === dateStr && e.originalTeacherId === T3.id).slice(0, 1);
    if (kamalEntries.length) {
      const absence2: Absence = {
        id: this.id(), teacherId: T3.id, date: dateStr, wholeDay: false,
        periodIds: kamalEntries.map((e) => e.periodId), reason: 'Personal Leave', remarks: 'Family event.',
      ...
      this.reliefAssignments.push({
        id: this.id(), absenceId: absence2.id, dailyEntryId: entry.id, reliefTeacherId: T14.id,
        assignedBy: 'Admin User', assignedAt: new Date().toISOString(), score: 90,
      });
    ...
    if (priyaToday) {
      ...
      this.lessonPlans.push({
        ...
        resources: 'Textbook Ch. 4, fraction strips, whiteboard', homework: 'Exercise 4C, questions 1-10',
    ...
    if (kamalToday) {
      this.lessonPlans.push({
        ...
        status: 'IN_PROGRESS', progressNotes: 'Practical demonstration underway; write-up to be finished next period.',
    ...
    if (completedSrc) {
      this.lessonPlans.push({
        ...
        resources: 'Place value chart, textbook Ch. 1', homework: 'Exercise 1B',
        status: 'COMPLETED', progressNotes: 'Covered fully; class did well on the quick check.',
  ...
  private generateMasterTimetable(slotTemplateType: Map<string, SlotType>) {
    for (const [slotId, templateType] of slotTemplateType) {
      ...
      for (const cls of this.classes) {
        this.masterTimetable.push({ id: this.id(), day: slot.day, periodId: slot.id, classId: cls.id, type: templateType, locked: false });
      }
    }
  ...
  private fillLessonsOnce(targetClasses: SchoolClass[]) {
    ...
    const gradeSubjectConf = (gradeId: string, subjectId: string) => this.gradeSubjects.find((g) => g.gradeId === gradeId && g.subjectId === subjectId);
    const qualifiedTeachers = (subjectId: string, classId: string, gradeId: string) =>
      this.teachers.filter((t) => t.active && t.subjectIds.includes(subjectId) && (t.classIds.length === 0 ? t.gradeIds.includes(gradeId) : t.classIds.includes(classId)));
    ...
    const perDayCount = new Map<string, Map<string, number>>();
    ...
    for (const day of shuffledDays) {
      ...
      for (const slot of slots) {
        const rotate = targetClasses.length ? (slot.order + WORKING_DAYS.indexOf(day) + runSeed) % targetClasses.length : 0;
        ...
        for (const cls of ordered) {
          ...
          const candidates = [...remMap.entries()].filter(([, c]) => c > 0).sort((a, b) => b[1] - a[1] || Math.random() - 0.5);
          ...
          for (const [subjectId] of candidates) {
            ...
            const freeTeacher = [...qualifiedTeachers(subjectId, cls.id, cls.gradeId)]
              .filter((t) => !teacherBusy.has(`${t.id}|${slot.id}`) && !t.unavailable.some((u) => u.day === day && u.periodId === slot.id))
            ...
            if (subject.requiresSpecialRoom) {
              const specialRoom = this.rooms.find(
                (r) => (subject.code === 'ICT' && r.name === 'ICT Lab') || (subject.code === 'SCI' && r.name === 'Science Lab') || (subject.code === 'PE' && r.name === 'Main Hall'),
              );
            ...
            this.masterTimetable.push({ id: this.id(), day, periodId: slot.id, classId: cls.id, type: 'LESSON', subjectId, teacherId: freeTeacher.id, roomId, locked: false });
            occupied.add(occKey);
    ...
    for (const cls of targetClasses) {
      for (const [subjectId, count] of remaining.get(cls.id)!) {
        if (count > 0) unresolved.push(`${cls.name}: ${subjectById.get(subjectId)?.name ?? subjectId} still needs ${count} more period(s)/week`);
      }
    }
  ...
  materializeDailyTimetable(date: string, day: Day) {
    ...
    for (const m of entries) {
      this.dailyTimetable.push({
        id: this.id(), date, day, periodId: m.periodId, classId: m.classId, type: m.type, subjectId: m.subjectId,
        teacherId: m.teacherId, originalTeacherId: m.teacherId, roomId: m.roomId, status: 'SCHEDULED', isRelief: false, masterEntryId: m.id,
      });
    }
  }
}
