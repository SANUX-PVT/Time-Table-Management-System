...
export interface Teacher {
  ...
  unavailable: { day: Day; periodId: string }[];
  active: boolean;
}
...
export interface LessonPlanSummaryRow {
  teacherId: string;
  teacherName: string;
  total: number;
  planned: number;
...
export interface DashboardSummary {
  date: string;
  teachers: { present: number; absent: number; pending: number; total: number };
  lessons: { scheduled: number; completed: number; inProgress: number };
  relief: { required: number; assigned: number; pending: number };
  alerts: { level: 'red' | 'yellow' | 'green'; message: string }[];
}
